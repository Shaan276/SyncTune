  ...
  return (
    <header className="flex flex-wrap items-center justify-between gap-4 p-4 mb-2">
      ...
      <form onSubmit={handleSearchSubmit} className="flex-1 min-w-[320px] max-w-2xl flex flex-col gap-2">
        <div className="flex items-center gap-3 px-4 py-2.5 rounded-xl bg-black/30 border border-white/10 focus-within:border-purple-500/50 transition-all shadow-inner">
          <Search className="w-4 h-4 text-zinc-400" />
          <input
            type="text"
            placeholder="Search tracks, artists, or mixes..."
            value={query}
          ...
          <button
            type="submit"
            className="px-3 py-1 rounded-lg bg-gradient-to-r from-purple-600 to-cyan-600 text-white font-bold text-xs hover:opacity-90 transition-opacity"
          >
            Search
          </button>
        </div>
